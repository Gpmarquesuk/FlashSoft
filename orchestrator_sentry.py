...
class Orchestrator:
    """
    Sentry Orchestrator (autopilot):
      - Roda run.py como subprocesso
      - Salva stdout/stderr em logs/ORCH_<run>_out.txt
      - Lê logs/run-*.jsonl e infere estado
      - Políticas:
          * Sem progresso (apenas router_models/start) => FORCE_JSON=1
          * Persistindo sem progresso => troca planner para fallback (env MODEL_PLANNER=fallback)
          * Branch já existe => adiciona timestamp
          * Push rejeitado => git fetch + pull --rebase
          * Pytest falhou => uma nova tentativa
      - Para quando abre PR ou após max tentativas.
      - Relatório em logs/ORCH_<run>.md
    """
    ...
    def _parse_outcome(self, log_path: Path):
        ...
        try:
            ...
            pass
        return {"pr": pr, "pytest_ok": pytest_ok, "events": [e for e in events if e]}
    ...
    def _write_report(self, run_id: str, decisions: list, outcome: dict, outlog: Path | None):
    ...
    def _decide(self, combined_text: str, outcome: dict, context: dict):
        ...
        # 1) early crash: só router_models + start
        ...
        # 2) mensagens explícitas
        t = text.lower()
        if "already exists" in t and "branch" in t:
    ...
    def run(self, spec_path: str, branch: str, max_attempts: int = 3, extra_env: dict | None = None):
        ...
        # pegar fallback do planner do .env (se houver)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # salvar stdout/stderr para auditoria
        ...
        latest = self._latest_run_log()
        outcome = self._parse_outcome(latest) if latest else {}
        self._write_report(run_label, decisions, outcome, outlog if 'outlog' in locals() else None)
