...
def test_existing_files_are_reported_as_skipped(tmp_path: Path):
    ...
    normalized = {path.replace("\\", "/") for path in _normalize_relative_paths(skipped_paths, workspace)}
    assert "src/core/service.py" in normalized, "Existing file should be reported as skipped."


def test_scaffolder_does_not_mutate_plan(tmp_path: Path):
    plan = _make_plan()
    original = json.loads(json.dumps(plan))

    result, workspace = _run_scaffolder(plan, tmp_path)
    created_paths = _extract_paths_from_result(result, "created")
    normalized = {path.replace("\\", "/") for path in _normalize_relative_paths(created_paths, workspace)}

    assert plan == original
    assert "src/core/service.py" in normalized


def test_path_to_summary_normalizes_windows_paths(tmp_path: Path):
    base = tmp_path
    target = tmp_path / "pkg" / "module.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("# sample\n", encoding="utf-8")

    summary = scaffolder._path_to_summary(base, target)
    assert summary == "pkg/module.py"

    external = Path("C:/flashsoft/outside/module.py")
    external_summary = scaffolder._path_to_summary(base, external)
    assert "\\" not in external_summary
