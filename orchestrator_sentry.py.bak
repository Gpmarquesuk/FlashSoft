import os, sys, json, time, uuid, subprocess, traceback
...
class Orchestrator:
    ...
    def _ensure_files(self):
        prompts = self.repo / "prompts"; prompts.mkdir(parents=True, exist_ok=True)
    ...
    def _parse_outcome(self, log_path: Path):
        ...
        try:
            for line in log_path.read_text(encoding="utf-8").splitlines():
                try:
                    obj=json.loads(line); ev=obj.get("event")
    ...
    def _write_report(self, run_id: str, decisions: list, outcome: dict, outlog):
        ...
        lines = [f"# Orchestrator Report — {datetime.utcnow().isoformat()}Z",
                 f"- run_id: `{run_id}`",
                 f"- outlog: `{outlog.name if outlog else '-'}`",
                 "## Decisions"]
        for i,d in enumerate(decisions,1): lines.append(f"{i}. action={d['action']} reason={d['reason']}")
        lines.append("## Outcome"); lines.append(json.dumps(outcome, ensure_ascii=False, indent=2))
        md.write_text("\n".join(lines), encoding="utf-8")

    # -------- decision policy --------
    def _decide(self, combined_text: str, outcome: dict, context: dict):
        text=(combined_text or ""); t=text.lower(); events=outcome.get("events") or []
        ...
        if "jsondecodeerror" in t or "saída sem json" in t or "resposta sem json" in t:
            if not context.get("forced_json"): return "force_json","planner non-JSON -> FORCE_JSON=1"
        if "pytest" in t and "failed" in t: return "retry","pytest failed -> retry once"
        if events and set(events).issubset({"router_models","start"}):
            if not context.get("forced_json"): return "force_json","early-crash/no progress -> FORCE_JSON=1"
            if not context.get("switched_planner"): return "switch_planner","no progress after FORCE_JSON -> switch planner to fallback"
    ...
    def run(self, spec_path: str, branch: str, max_attempts: int = 3, extra_env: dict | None = None):
        ...
        decisions=[]; ctx={"forced_json":False,"switched_planner":False}
        ...
        for attempt in range(1,max_attempts+1):
            ...
            # imprime uma amostra do erro em tempo real (sem babysitting)
            try:
                tail = "\n".join((combined.strip().splitlines() or [])[-20:])
                if tail: print(f"[orch][attempt {attempt}] tail:\n{tail}\n")
            except Exception: pass
            ...
            action,reason=self._decide(combined,outcome,ctx); decisions.append({"action":action,"reason":reason})
            if action=="force_json": ctx["forced_json"]=True; continue
            if action=="switch_planner": ctx["switched_planner"]=True; continue
            if action=="new_branch_suffix":
                ts=datetime.utcnow().strftime("%Y%m%d-%H%M%S"); cur_branch=f"{branch}-{ts}"; continue
            if action=="git_sync": self._git_sync(); continue
            # retry / retry_unknown -> apenas loop

        latest=self._latest_run_log()
        outcome=self._parse_outcome(latest) if latest else {}
        self._write_report(run_label,decisions,outcome,outlog)
        # despeja o final do outlog pra você ver na hora
